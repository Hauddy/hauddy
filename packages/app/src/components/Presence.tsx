...
/** Honest local/network state tag — network reach is a property of a contact,
 *  not a separate world. */
export function OriginTag({ origin }: { origin: 'local' | 'network' }) {
  return <span className={`origin-tag origin-${origin}`}>{origin}</span>;
}
